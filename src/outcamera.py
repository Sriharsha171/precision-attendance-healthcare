...
def identify_faces(video_capture):
    ...
    while True:
        ...
        for name, (top, right, bottom, left) in predictions:
            ...
            if name!="UNKNOWN" or face_names!="unknown" or face_names!="UnKnown":
                dname=""
                intime=""
                outtime=""
                currentDateAndTime = datetime.datetime.now()
                currentTime = currentDateAndTime.strftime("%H:%M:%S")
                query = "SELECT intime FROM attendance WHERE dat = ? and name=?"
                data = cursor.execute(query, (current_date, name))
                row = data.fetchone()
                
                # Check if row is None before accessing it
                if row:
                    intime = row[0]  # Extract intime from the row
                    t1 = datetime.datetime.strptime(intime, "%H:%M:%S")
                    t2 = datetime.datetime.strptime(currentTime, "%H:%M:%S")
                    delta = t2 - t1
                    query = "UPDATE attendance SET outtime=?, workhours=? WHERE name=? AND dat=?"
                    cursor.execute(query, (str(t2), str(delta), str(name), current_date))
                    conn.commit()
                    break
                else:
                    print(f"No records found for {name} on {current_date}")
            else:
